...
async function runElectronBuilderRaw(
  ...
  try {
    ...
    if (isWinCodeSignSymlinkPrivilegeError(error)) {
      ...
      await runSegment("electron-builder-raw:retry-without-executable-editing-prepare-config", async () => {
        await removeTree(paths.appBuilderOutputRoot);
        await writeBuilderConfig();
      });
      await build("electron-builder-raw:process-without-executable-editing");
      return segments;
    }
